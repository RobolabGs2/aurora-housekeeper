...
export class FireballSystem {
	...
	spawnFireball<T extends { x: number; y: number }>(
	...
	update(dt: number) {
		...
		for (const f of this.fireballsGroup.getChildren() as Phaser.GameObjects.Sprite[]) {
...
function moveSoundInPoint(
	...
	const dist = Phaser.Math.Clamp(
		Phaser.Math.Distance.Between(centerX, centerY, f.x, f.y) / 512,
		0.001,
		1
	);
